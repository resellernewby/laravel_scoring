...
<div x-data="{ open: false }" @keydown.window.escape="open = false" @click.away="open = false"
    ...
    <div>
        <span class="rounded-md shadow-sm">
            <button @click="open = !open" type="button" {{ $attributes->merge(["class" => "inline-flex justify-center
                w-full rounded-md border border-gray-200 px-4 py-2 bg-white text-sm leading-5
                font-medium text-gray-700 hover:text-gray-500 focus:outline-none focus:border-blue-300
    ...
    <div x-show="open" style="display: none;" x-description="Dropdown panel, show/hide based on dropdown state."
        ...
        <div class="rounded-md bg-white shadow-xs">
            <div class="py-1 {{ $divider ? 'px-2 divide-y divide-gray-100 space-y-4' : '' }}" role="menu"
