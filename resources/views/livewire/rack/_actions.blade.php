<x-dropdown label="Opsi" wire:key="button-{{ $rack->id }}">
    <x-dropdown.item class="flex"
        onclick="Livewire.emit('openModal', 'rack.edit', {{ json_encode(['rack' => $rack->id]) }})">
    ...
    @if ($isDelete)
        <div class="px-4">
            <x-button.confirmation yes="destroy({{ $rack->id }})" no="$set('isDelete', false)" />
        </div>
    @else
