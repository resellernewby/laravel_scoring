<div>
    <x-modal form-action="store">
        <x-slot name="title">
            Tambah subrak <strong>{{ $rack->name }}</strong>
        </x-slot>

        <x-slot name="content">
            <div class="flex flex-col space-y-4">
                <x-input label="Nama subrak" wire:model.lazy="inputs.name" :error="$errors->first('inputs.name')" />

                <x-textarea label="Deskripsi" wire:model.lazy="inputs.description"
