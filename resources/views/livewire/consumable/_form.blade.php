    ...
    @foreach ($storages as $key => $input)
        <div class="bg-gray-50 border-dashed border-2 border-indigo-600 rounded-md p-2"
            ...
            <div class="mt-4 flex items-center space-x-4">
                <div class="w-3/5">
                    <x-select id="rack_{{ $key }}" label="Rak*" wire:model.lazy="rack.{{ $key }}.id"
                        :list="$rackLists[$key] ?? null" :error="$errors->first('rack.{{ $key }}.id')" required />
                </div>
...
<div class="lg:w-3/5">
    ...
    <div class="mt-4 space-y-4">
        @foreach ($spec as $k => $i)
            <div class="flex space-x-4 bg-gray-50 rounded-md p-2" wire:key="spec-{{ $k }}">
                <div class="w-full">
                ...
                <div class="w-full">
                    <x-input id="spec_{{ $k }}_value" label="Isi keterangan"
                        wire:model.lazy="spec.{{ $k }}.value" :error="$errors->first('spec.{{ $k }}.value')" placeholder="mis. 10 Kg" />
                </div>
