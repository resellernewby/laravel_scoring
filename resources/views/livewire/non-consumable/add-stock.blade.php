            ...
            <form wire:submit.prevent="store">
                <div class="sm:flex sm:space-x-8 sm:justify-between sm:px-6 sm:py-4 px-3 py-3.5">
                    <div class="w-2/5 flex flex-col space-y-4">
                        ...
                        <x-input label="Model" value="{{ $asset->model }}" disabled />
                        ...
                        <x-input.addon-right label="Masa ekonomis" wire:model.lazy="nonconsumable.economic_age"
                            leading-add-on="Bulan" help-text="Masa ekonomis dalam bulan" :error="$errors->first('nonconsumable.economic_age')" />

                        <x-input.money label="Nilai residu" leading-add-on="Rp"
                            wire:model.lazy="nonconsumable.residual_value" :error="$errors->first('nonconsumable.residual_value')"
                            help-text="Harga/nilai sisa aset yang sudah berakhir umur ekonomisnya" />

                        <x-input.addon-right label="Masa garansi" wire:model.lazy="nonconsumable.warranty_period"
                            leading-add-on="Bulan" help-text="Masa garansi dalam bulan" :error="$errors->first('nonconsumable.warranty_period')" />

                        <x-input label="Penyedia garansi" wire:model.lazy="nonconsumable.warranty_provider"
                            :error="$errors->first('nonconsumable.warranty_provider')" />
                    </div>
