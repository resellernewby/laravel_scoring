    ...
    <div class="-mx-4 my-10 shadow bg-white sm:-mx-6 md:mx-0 md:rounded-lg">
        <div class="sm:flex sm:space-x-4 sm:items-center sm:justify-between sm:px-6 sm:py-4 px-3 py-3.5">
            ...
            <div class="flex items-center space-x-2">
                <x-button.primary onclick="Livewire.emit('openModal', 'classroom.create')"
        ...
        <x-table>
            ...
            <x-slot name="body">
                @forelse ($classrooms as $classroom)
                    <tr wire:key="classroom-{{ $classroom->id }}" wire:loading.class.delay="opacity-50">
                        <td
                            class="relative py-4 pl-4 sm:pl-6 pr-3 text-sm {{ !$loop->first ? 'border-t border-transparent' : '' }}">
                            <div>
                                <span class="font-medium text-gray-900">{{ $classroom->name }}</span>
                            </div>
                        ...
                        <td
                            ...
                            @include('livewire.classroom._actions')
                            @if (!$loop->first)
